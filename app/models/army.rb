class Army
  ...
  after_save :recalculate_max_weight_of_resources
  ...
  validates :max_weight, numericality: { greater_than_or_equal_to: 0 }
  ...
  def recalculate_max_weight_of_resources
    weight_attribute_id = world.config(:weight_attribute_id).weight_attribute_id
    max_weight_for_army = regiments.groups.map do |group|
